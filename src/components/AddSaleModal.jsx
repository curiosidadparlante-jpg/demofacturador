...
export default function AddSaleModal({ isOpen, onClose, onSave, searchClientes }) {
  ...
  const handleCuitLookup = async (rawCuit) => {
    ...
    try {
      ...
      const mockRazonSocial = isCompany ? `Empresa Demo ${val.substring(2, 6)} SA` : `Usuario ${val.substring(2, 6)}`;
      const mockCondicion = isCompany ? 'IVA Responsable Inscripto' : 'Consumidor Final';
